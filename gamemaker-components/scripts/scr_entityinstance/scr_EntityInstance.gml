...
function EntityInstanceSystem(_world) : ComponentSystem(_world) constructor {
	...
	*/
	...
	function SetSize(_entityId, _xWidth, _yHeight) {
		...
		var eInst = entityRef.components.entityInstance;
		eInst.maskWidth = _xWidth;
		eInst.maskHeight = _yHeight;
		eInst.image_xscale = _xWidth / sprite_get_width(eInst.maskIndex);
		eInst.image_yscale = _yHeight / sprite_get_height(eInst.maskIndex);
	...
	function SetImageAngle(_entityId, _rotationDegrees) {
		var entityRef = entity.GetRef(_entityId);
		var eInst = entityRef.components.entityInstance;
		eInst.imageAngle = _rotationDegrees;
		//TODO forward to inst props?
		//eInst.instanceRef.image_angle = _rotationDegrees;
	}
	
	function SetImageScale(_entityId, _xScale, _yScale) {
		var entityRef = entity.GetRef(_entityId);
		var eInst = entityRef.components.entityInstance;
		eInst.imageXscale = _xScale;
		eInst.imageYscale = _yScale;
		//TODO forward to inst props?
	}
