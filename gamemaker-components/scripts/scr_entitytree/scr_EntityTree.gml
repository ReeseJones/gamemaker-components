...
/// @param {Real}    _parentEntityId Id of the entity to be a parent.
    static setParent = function(_childEntityId, _parentEntityId) {
        var _childEntityRef = world.getRef(_childEntityId);
        var _parentEntityRef = _parentEntityId ? world.getRef(_parentEntityId) : undefined;
        ...
        var _childEntityTree = _childEntityRef.component.entityTree;
        ...
        if(_parentEntityRef) {
            var _otherEntityTree = _parentEntityRef.component.entityTree;
            if(!_otherEntityTree) {
        ...
        var _oldParentRef = world.getRef(_childEntityTree.parent);
        if(_oldParentRef) {
            array_remove_first(_oldParentRef.component.entityTree.children, _childEntityId);
        }
...
/// @param {Real}    _childEntityId Child entity id.
    static addChild = function(_entityId, _childEntityId) {
...
/// @param {Real}   _entityId Entity Id to remove the entity tree that its in.
    static disconnectFromTree = function(_entityId) {
        var _entityRef = world.getRef(_entityId);

        if(is_undefined(_entityRef)) {
            throw string_join("", "Entity with id ", _entityRef , " does not exist for disconnectFromTree.");    
        }
        
        var _entityTreeComp = _entityRef.component.entityTree;
        ...
        //Disconnect parent
        var _parentRef = world.getRef(_entityTreeComp.parent);
        if(_parentRef) {
            var _parentEntityTree = _parentRef.component.entityTree;
        ...
        array_foreach(_entityTreeComp.children, function(_entityId) {
            var _entityRef = world.getRef(_entityId);
            _entityRef.component.entityTree.parent = undefined;
        });
...
/// @param {Bool}       _skipSelf Whether to include the root entity in the traversal
    static foreachEntityDown = function(_entityId, _callback, _skipSelf = false) {
        ds_queue_clear(childTraversalQueue);
        
        var _queueEntity = function(_entityId) {
            ds_queue_enqueue(childTraversalQueue, _entityId);
        };

        var _entityRef = world.getRef(_entityId);
        var _entityTree = _entityRef.component.entityTree;
        if(_skipSelf) {
        ...
        while(!ds_queue_empty(childTraversalQueue)) {
            ...
            var _currentRef = world.getRef(_currentEntityId);
            var _currentEntityTree = _currentRef.component.entityTree;
...
/// @param {Function}   _callback Method which is called on each entity. callback(entity, entityId)
/// @param {Bool}       _skipSelf Whether to include the root entity in the traversal
    static foreachEntityUp = function(_entityId, _callback, _skipSelf = false) {
        var _entityRef = world.getRef(_entityId);
        var _entityTree = _entityRef.component.entityTree;
        if(_skipSelf) {
        ...
        while(_entityId) {
            var _currentEntity = world.getRef(_entityId);
            var _currentEntityTree = _currentEntity.component.entityTree;
            if(_currentEntityTree) {
...
/// @param {Real}   _entityId Entity Id to remove the entity tree that its in.
    static entityDestroyTree = function(_entityId) {
        foreachEntityDown(_entityId, function(_entityRef, _entityId) {
            world.destroyEntity(_entityId);
        });
    }
